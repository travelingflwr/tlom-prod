import listActions from 'modules/mylomtest/list/mylomTestListActions';
...
const actions = {
  ...
  doDestroy: (id) => async (dispatch) => {
    try {
      ...
      await MylomTestService.destroyAll([id]);

      dispatch({
        type: actions.DESTROY_SUCCESS,
      });

      Message.success(i18n('entities.mylomtest.destroy.success'));
  ...
  doDestroyAll: (ids) => async (dispatch) => {
    try {
      ...
      await MylomTestService.destroyAll(ids);
      ...
      Message.success(
        i18n('entities.mylomtest.destroyAll.success'),
      );
