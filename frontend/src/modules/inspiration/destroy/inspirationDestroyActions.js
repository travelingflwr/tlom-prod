import listActions from 'modules/inspiration/list/inspirationListActions';
...
const actions = {
  ...
  doDestroy: (id) => async (dispatch) => {
    try {
      ...
      await InspirationService.destroyAll([id]);

      dispatch({
        type: actions.DESTROY_SUCCESS,
      });

      Message.success(i18n('entities.inspiration.destroy.success'));
  ...
  doDestroyAll: (ids) => async (dispatch) => {
    try {
      ...
      await InspirationService.destroyAll(ids);
      ...
      Message.success(
        i18n('entities.inspiration.destroyAll.success'),
      );
