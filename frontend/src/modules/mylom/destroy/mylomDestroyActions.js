import listActions from 'modules/mylom/list/mylomListActions';
...
const actions = {
  ...
  doDestroy: (id) => async (dispatch) => {
    try {
      ...
      await MylomService.destroyAll([id]);

      dispatch({
        type: actions.DESTROY_SUCCESS,
      });

      Message.success(i18n('entities.mylom.destroy.success'));
  ...
  doDestroyAll: (ids) => async (dispatch) => {
    try {
      ...
      await MylomService.destroyAll(ids);
      ...
      Message.success(
        i18n('entities.mylom.destroyAll.success'),
      );
