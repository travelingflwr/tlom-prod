...
import selectors from 'modules/tag/tagSelectors';

class TagViewItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    if (this.props.hasPermissionToRead) {
      return (
        <div key={record.id}>
          <Link className="btn btn-link" to={`/tag/${record.id}`}>
            {record['id']}
          </Link>
        </div>
      );
...
export default connect(select)(TagViewItem);
