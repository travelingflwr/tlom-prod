...
import actions from 'modules/hive/list/hiveListActions';
import selectors from 'modules/hive/list/hiveListSelectors';
...
class HiveToolbar extends Component {
  state = {
    ...
    if (disabled) {
      return (
        <span
          ...
          data-for="hive-list-toolbar-export-tooltip"
        >
          {button}
          <ReactTooltip id="hive-list-toolbar-export-tooltip" />
        </span>
      );
    }
    ...
    if (disabled) {
      return (
        <span
          ...
          data-tip-disable={!disabled}
          data-for="hive-list-toolbar-destroy-tooltip"
        >
          {button}
          <ReactTooltip id="hive-list-toolbar-destroy-tooltip" />
        </span>
      );
    }
    ...
    return (
      <Toolbar>
        ...
        {this.props.hasPermissionToAuditLogs && (
          <Link to="/audit-logs?entityNames=hive">
...
function select(state) {
  return {
    ...
    hasPermissionToCreate: hiveSelectors.selectPermissionToCreate(
      state,
    ),
    hasPermissionToImport: hiveSelectors.selectPermissionToImport(
      state,
    ),
  };
}

export default connect(select)(HiveToolbar);
