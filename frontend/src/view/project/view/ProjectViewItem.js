...
import selectors from 'modules/project/projectSelectors';

class ProjectViewItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    if (this.props.hasPermissionToRead) {
      return (
        <div key={record.id}>
          <Link className="btn btn-link" to={`/project/${record.id}`}>
            {record['name']}
          </Link>
        </div>
      );
...
export default connect(select)(ProjectViewItem);
