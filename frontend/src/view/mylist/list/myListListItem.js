...
class myListListItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    if (this.props.hasPermissionToRead) {
      return (
        <div key={record.id}>
          <Link className="btn btn-link" to={`/mylist/${record.id}`}>
            {record['name']}
          </Link>
        </div>
      );
...
export default connect(select)(myListListItem);
