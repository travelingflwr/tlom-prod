...
import SubscriptionForm from 'view/subscription/form/SubscriptionForm';
import SubscriptionService from 'modules/subscription/subscriptionService';
import Errors from 'modules/shared/error/errors';

class SubscriptionFormModal extends Component {
  constructor(props) {
    super(props);
  ...
  doSubmit = async (_, data) => {
    try {
      ...
      const { id } = await SubscriptionService.create(data);
          ...
          <div className="modal-content">
            <div className="modal-header">
              <h5 className="modal-title">
                {i18n('entities.subscription.new.title')}
              </h5>
              <button
                type="button"
                className="close"
