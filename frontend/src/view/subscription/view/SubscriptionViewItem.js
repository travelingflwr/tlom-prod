...
class SubscriptionViewItem extends Component {
  valueAsArray = () => {
  ...
  displayableRecord = (record) => {
    if (this.props.hasPermissionToRead) {
      return (
        <div key={record.id}>
          <Link className="btn btn-link" to={`/subscription/${record.id}`}>
            {record['name']}
          </Link>
        </div>
      );
...
SubscriptionViewItem.propTypes = {
...
export default connect(select)(SubscriptionViewItem);
