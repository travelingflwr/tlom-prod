...
class TaskRepository {
  ...
  async create(data, options) {
    ...
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(
          `${new Task().collectionName}/${record.id}`,
        ),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Task().collectionName}/${id}`),
      null,
      options,
    );
  ...
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Task().collectionName);
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.parent) {
        query.appendId('parent', filter.parent);
      }
  ...
  async populate(record) {
    ...
    record.parent = await FirebaseRepository.findRelation(
      'task',
      record.parent,
    );
