...
/**
 * Handles database operations for the Inspiration.
 */
class InspirationRepository {
  ...
  async create(data, options) {
    ...
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(
          `${new Inspiration().collectionName}/${record.id}`,
        ),
      record,
      options,
    );
    ...
    await FirebaseRepository.refreshTwoWayRelationManyToMany(
      record,
      'tag',
      'tag',
      'inspiration',
  ...
  async destroy(id, options) {
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Inspiration().collectionName}/${id}`),
      null,
      options,
    );
  ...
  /**
   * Counts the number of Inspirations based on the filter.
   *
   * @param {Object} filter
   */
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Inspiration().collectionName);
  ...
  /**
   * Finds the Inspiration and its relations.
   *
   * @param {string} id
   */
  async findById(id) {
  ...
  /**
   * Finds the Inspirations based on the query.
  ...
  /**
   * Lists the Inspirations to populate the autocomplete.
  ...
  async populate(record) {
    ...
    record.tag =
      (await FirebaseRepository.findRelation('tag', record.tag)) ||
      [];
  ...
  async _createAuditLog(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Inspiration().modelName,
        entityId: id,
        action,
        values: data,
      },
...
module.exports = InspirationRepository;
