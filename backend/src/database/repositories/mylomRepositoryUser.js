...
const Mylom = require('../models/mylom');
const AuditLogRepository = require('./auditLogRepository');

/**
 * Handles database operations for the Mylom.
 */
class MylomRepositoryUser {
  /**
   * Creates the Mylom.
   *
   * @param {Object} data
  ...
  async create(data, options) {
    ...
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(
          `${new Mylom().collectionName}/${record.id}`,
        ),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Mylom().collectionName}/${id}`),
      null,
      options,
    );
  ...
  /**
   * Counts the number of Myloms based on the filter.
   *
   * @param {Object} filter
   */
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Mylom().collectionName);
  ...
  /**
   * Finds the Mylom and its relations.
   *
   * @param {string} id
   */
  async findById(id) {
  ...
  /**
   * Finds the Myloms based on the query.
  ...
  /**
   * Lists the Myloms to populate the autocomplete.
  ...
  async findUserAutocomplete() {
    var userOwnedRef = db.collection("mylom");
    console.log(userOwnedRef);
    var query = userOwnedRef.where("user", "==", FirebaseRepository.getCurrentUser());
  ...
  async _createAuditLog(action, id, data, options) {
    await AuditLogRepository.log(
      {
        entityName: new Mylom().modelName,
        entityId: id,
        action,
        values: data,
      },
...
module.exports = MylomRepositoryUser;
