...
/**
 * Handles database operations for the Hive.
 */
class HiveRepository {
  ...
  async create(data, options) {
    ...
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(
          `${new Hive().collectionName}/${record.id}`,
        ),
      record,
      options,
    );
  ...
  async destroy(id, options) {
    await FirebaseRepository.executeOrAddToBatch(
      ...
      admin
        .firestore()
        .doc(`${new Hive().collectionName}/${id}`),
      null,
      options,
    );
  ...
  /**
   * Counts the number of Hives based on the filter.
   *
   * @param {Object} filter
   */
  async count(filter) {
    let chain = admin
      .firestore()
      .collection(new Hive().collectionName);
  ...
  /**
   * Finds the Hive and its relations.
   *
   * @param {string} id
   */
  async findById(id) {
  ...
  /**
   * Finds the Hives based on the query.
  ...
  async findAndCountAll(
    ...
    if (filter) {
      ...
      if (filter.mylom) {
        query.appendId('mylom', filter.mylom);
      }
  ...
  /**
   * Lists the Hives to populate the autocomplete.
  ...
  async populate(record) {
    ...
    record.mylom = await FirebaseRepository.findRelation(
      'mylom',
      record.mylom,
    );
